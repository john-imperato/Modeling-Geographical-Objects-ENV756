...
input_gdb = rf"{base_dir}\Input.gdb" # input gdb
output_gdb = rf"{base_dir}\Data_Automation.gdb"  # output gdb

# list of input species feature classes
species_list = [
    rf"{input_gdb}\YoToad",
    rf"{input_gdb}\SNYLF",
    rf"{input_gdb}\Bighorn",
    rf"{input_gdb}\MYLF"
]

# corresponding common names for each species
common_names = ["Species1", "Species2", "Species3", "Species4"]

# ecological degradation source feature class
source_of_degradation = rf"{input_gdb}\trails_clipped"

# buffer distances (meters)
buffer_distances = [500, 1500, 3000]

# buffer values
buffer_values = {500: 5, 1500: 3, 3000: 1}

# dictionary to store paths for dissolved feature classes
dissolved_fc_paths = {}

# loop through zipped feature classes/common names lists
for fc, common_name in zip(feature_classes, common_names):
