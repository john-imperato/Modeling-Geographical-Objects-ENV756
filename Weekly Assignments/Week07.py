...
df = pd.read_csv(r"C:\Modeling Geographical Objects\MossyPond\Data\capture.csv")

# create new 'Year' variable
df['Year'] = pd.to_datetime(df['Date']).dt.year

# Save modified df to new CSV file
df.to_csv(r"C:\Modeling Geographical Objects\MossyPond\Data\capture_year.csv", index=False)

# create new GDB to store table & feature class
arcpy.management.CreateFileGDB(r"C:\Modeling Geographical Objects\MossyPond", "GDB_MP.gdb")

# set paths for:
csv_path =  (r"C:\Modeling Geographical Objects\MossyPond\Data\capture_year.csv")
gdb_path = r"C:\Modeling Geographical Objects\MossyPond\GDB_MP.gdb" # geodatabase
standalone_table = r"C:\Modeling Geographical Objects\MossyPond\GDB_MP.gdb\capture_table"  # standalone table
output_fc = r"C:\Modeling Geographical Objects\MossyPond\GDB_MP.gdb\captures_points" # feature class

# Create standalone table
arcpy.conversion.TableToTable(csv_path, gdb_path, "capture_table")

# change data type for UTM variables 
# add new numeric fields as DOUBLE
arcpy.management.AddField(standalone_table, "utmE_frog_num", "DOUBLE")
arcpy.management.AddField(standalone_table, "utmN_frog_num", "DOUBLE")

# calculate UTM Easting (utmE_frog) as a float, handling 'NA' values
arcpy.management.CalculateField(
    standalone_table,
    "utmE_frog_num",
    "float(!utmE_frog!) if !utmE_frog! not in ['NA', None, ''] else None",
    "PYTHON3"
)

# Calculate UTM Northing (utmN_frog) as a float, handling 'NA' values
arcpy.management.CalculateField(
    standalone_table,
    "utmN_frog_num",
    "float(!utmN_frog!) if !utmN_frog! not in ['NA', None, ''] else None",
    "PYTHON3"
)

# Create a point feature class from the standalone table
arcpy.management.XYTableToPoint(standalone_table, output_fc, "utmE_frog_num", 
                                "utmN_frog_num", coordinate_system="32610")

# define paths
points = r"C:\Modeling Geographical Objects\MossyPond\GDB_MP.gdb\captures_points"
output_raster = r"C:\Modeling Geographical Objects\MossyPond\GDB_MP.gdb\KD_captures"
...
KD = arcpy.sa.KernelDensity(
    ...
    cell_size=6.33990159999952 # calculated by the GUI
)

# save the output raster
KD.save(output_raster)


# define path for NASA DEM
DEM_path = r"C:\Modeling Geographical Objects\MossyPond\Data\DEM_MP.tif"

# clip the DEM to the Mossy Pond study area
clip_area = r"C:\Modeling Geographical Objects\Week05\GDBWeek05.gdb\LandCover_Total"
clipped_DEM = r"C:\Modeling Geographical Objects\MossyPond\GDB_MP.gdb\clipped_DEM_MP"
arcpy.management.Clip(DEM_path, clip_area, clipped_DEM)

DEM_points = r"C:\Modeling Geographical Objects\MossyPond\GDB_MP.gdb\DEM_to_points"

arcpy.conversion.RasterToPoint(clipped_DEM, DEM_points)


# input parameters
z_field = "grid_code"
IDW_elevation = r"C:\Modeling Geographical Objects\MossyPond\GDB_MP.gdb\IDW_elevation"

# run IDW
IDW_ele = arcpy.sa.Idw(
    in_point_features=DEM_points,
    z_field=z_field,
)

# save output raster
IDW_ele.save(IDW_elevation)
...
# set paths
IDW_snowpack_2017 = r"C:\Modeling Geographical Objects\MossyPond\GDB_MP.gdb\IDW_snowpack_2017"
snowpack_TIN_2017 = r"C:\Modeling Geographical Objects\MossyPond\GDB_MP.gdb\snowpack_TIN_2017"

# convert raster to TIN
snowpack_TIN_2017 = arcpy.ddd.RasterTin(IDW_snowpack_2017, snowpack_TIN_2017)
